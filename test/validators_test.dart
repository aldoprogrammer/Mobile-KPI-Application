...
void main() {
  group('Validators.requiredField', () {
    test('returns error when value is null or empty', () {
      expect(Validators.requiredField(null), 'Field is required');
      expect(Validators.requiredField('   '), 'Field is required');
    });
  ...
  group('Validators.minLength', () {
    test('returns error when value is too short', () {
      expect(Validators.minLength(null, 6), 'Field must be at least 6 characters');
